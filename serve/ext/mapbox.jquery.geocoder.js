!function($) {
    ...
    function geocode(e) {
        ...
        reqwest({
            url: 'http://open.mapquestapi.com/nominatim/v1/search?format=json&json_callback=callback&&limit=1&countrycodes=es&q=' + query + '&viewbox=-4.81,37.91,-4.70,37.84',
            type: 'jsonp',
        ...
        function success(resp) {
            ...
            if (!resp) {
                $this.find('#geocode-error').text('Vaya, no encontramos la dirección').fadeIn('fast');
                console.log(resp);
                return;
            }
